...
class SimpleLoginSecure
{
	...
	public function __construct()
	{
		...
		$this->autoresponder_from_name = $this->CI->config->item('autoresponders_from_name');
		$this->autoresponder_from_email = $this->CI->config->item('autoresponders_from_email');
	}
	...
	function is_logged_in_site_owner()
	{
		if ($this->CI->session->userdata('user_role') == 'site_owner')
		{
			return TRUE;
		}
	}
	...
	function reset_password($user_email)
	{
		...
		$this->CI->email->clear();
		$this->CI->email->from(SYSTEM_EMAIL, $this->autoresponder_from_name);
		$this->CI->email->to($user_email);
		$this->CI->email->subject($this->autoresponder_from_name.' - Reset your password');
